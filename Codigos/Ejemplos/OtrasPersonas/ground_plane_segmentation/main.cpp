...
int control_robot(OwSafeBuffer<string> * commbuf, double turn_angle, double min_dist_to_obstacle, double widest_angle, double min_dist, double min_angle);
 
static char data[] = "19s20t"; //variable to hold string to control speed and orientation of robot platform
...
int main(int argc, char** argv)
{
    ...
    //int frame_no=-1; //Katramados videos start from -1
    //int frame_no=0;
    ...
    if( p.init_specific_robot() != 0) return 1; //set robot id in parameters 
    cout << "robot -> " << p.robot_names[p.robot_id] << "\n";
    ...
    CommLoop udp(p.robot_id,p.verb,p.network,p.net.ip1,p.net.ip2,p.net.ip3,p.net.ip4,p.net.port,p.debug, socketPtr);
    //udp.InitSocketUdp();
    std::thread comm_thread( std::thread( &CommLoop::Start, std::ref(udp), &commbuf, socketPtr ));
    
    //if (p.network) Bot.init_socket_udp(socketPtr, p.net.port); 
    // //if (p.network) Bot.init_comms(pPtr);
    // //init_socket_udp(socketPtr, p.net.port);
    ...
    while(1)
    {
        ...
        //      if( CheckConvergence(StatsPtr, em) ){
        //       
        //        em += 5;
        //      }
        
        //printf("em=%d\n",em);
        //if( CheckConvergence(StatsPtr, em) ) break;
        //cvWaitKey(1000);
        //}
        ...
        //     int T = 	cvRound((double)thres*0.1 - 30);
        ...
        //     sprintf(buffer1,"results/%s/%d/%d/%s_source_%d_%d.bmp", name[id-1],T,frame_no,name[id-1],frame_no,run);
        //     sprintf(buffer2,"results/%s/%d/%d/%s_result_%d_%d.bmp", name[id-1],T,frame_no,name[id-1],frame_no,run);
        ...
        /*
        cvNormalize( F.post_ratio,F.post_ratio, 0, 255, CV_MINMAX);
        cvConvertScaleAbs(F.post_ratio, prior, 1, 0);
        //cvMerge(F.post_ratio, F.post_ratio, F.post_ratio, 0, prior_rgb);
        cvCvtColor(prior, prior_rgb, CV_GRAY2BGR);
        //writeVideo(prior_rgb);
        */
        
        ////cvReleaseImage(&input_image);
        
        /*strcpy(data, "12s32t");
        socket.Send( Address(p.net.ip1,p.net.ip2,p.net.ip3,p.net.ip4,p.net.port), data, sizeof(data) );
        printf("SENDING UDP DATA: %s, on address: %d.%d.%d.%d:%d\n",data,p.net.ip1,p.net.ip2,p.net.ip3,p.net.ip4,p.net.port);
        */	
        ////if(p.robot) control_robot(socketPtr, RAD2DEG(Bound.D_Theta.y), Bound.D_Theta.x, RAD2DEG(Bound.widest_angle), Bound.min_distance, Bound.min_angular_separation);
        //if(p.robot) control_robot(socketPtr, RAD2DEG(Bound.D_Theta.y), Bound.D_Theta.x, RAD2DEG(Bound.widest_angle), Bound.min_distance, p.bot.min_ang_between_obst);
        if(p.robot) control_robot(&commbuf, RAD2DEG(Bound.D_Theta.y), Bound.D_Theta.x, RAD2DEG(Bound.widest_angle), Bound.min_distance, p.bot.min_ang_between_obst);
...
//int control_robot(net::Socket * socketPtr, double turn_angle, double min_dist_to_obstacle, double widest_angle, double min_dist, double min_angle)
int control_robot(OwSafeBuffer<string> * commbuf, double turn_angle, double min_dist_to_obstacle, double widest_angle, double min_dist, double min_angle)
{

    static double turn = 0.0;
    static double speed = 0.0;    
    turn = 0.4*turn + 0.6*turn_angle;

    //speed range -1-+1
    //steering range = degrees -90to90
    //scaled = ((mtx - MIN)./RANGE).*(MAXVAL-MINVAL) + MINVAL;
    if (p.verb) printf("turn angle = %0.2f, minD = %0.2f, widest=%0.2f, min_angle=%0.2f\n", turn, min_dist_to_obstacle, widest_angle, min_angle);

    if(p.move){
        //widest =   Bound.widest_angle*(180/3.14159);
        static int w = 1; //seems to be a counter to induce dela
        static int loop_delay = 1;
        if(w == loop_delay){ 
            if ((min_dist_to_obstacle >= 3*min_dist) 	&& (widest_angle > min_angle)) 						speed = 0.15;
            else if ((min_dist_to_obstacle < 	3*min_dist) 	&& (min_dist_to_obstacle >= 1.5*min_dist) && (widest_angle>min_angle) ) 	speed = 0.06;
            else if ((min_dist_to_obstacle < 	1.5*min_dist) 	&& (min_dist_to_obstacle >= 0.8*min_dist) && (widest_angle>min_angle)) 	speed = 0.02;
            else if ((min_dist_to_obstacle <= 0.8*min_dist) 	&& (widest_angle < min_angle ) ) 						speed = 0.0;
            else {speed = speed/2; turn = turn/2;};
                
            //Bot.send2robot(pPtr,socketPtr,turn,speed);
            Bot.send2robot(pPtr,commbuf,turn,speed);
            
            /*
            sprintf(data,"%ds%dt", angle2rover(turn), speed2rover(speed)+4);
            if (p.network) socketPtr->Send( net::Address(p.net.ip1,p.net.ip2,p.net.ip3,p.net.ip4,p.net.port), data, sizeof(data) );
            printf("SENDING UDP DATA: %s, on address: %d.%d.%d.%d:%d\n",data,p.net.ip1,p.net.ip2,p.net.ip3,p.net.ip4,p.net.port);
            */
            w=0; //reset counter
        }
        ...
        if(turn < 0.01 && speed < 0.01) p.move = 0;	      
    }
    else{
        if ( widest_angle < min_angle ){
        printf("CANNOT MOVE - ROTATION ROUTINE!\n");
            //playerc_position2d_set_cmd_vel(position2d, 0, 0, 0.3, 1);
            //rotate until robot finds a turning angle
            //strcpy(data, "75s51t"); //turn right
            //if (p.network) socketPtr->Send( net::Address(192,168,2,6,p.net.port), data, sizeof(data) );
        }
        if ( widest_angle >= min_angle ){
            p.move =1;
        }
